...
5. Đọc `outputs/comparison.csv`, `comparison_vlm.csv` và các hình `outputs/*.png`.
...
```
            ...
            ▼                                ▼
   ┌─────────────────────┐        ┌─────────────────────────┐
   │  NHÁNH A (vision)    │        │  NHÁNH B (VLM) — SONG    │
   │  run_all_methods     │        │  phase2_clip_adapt       │
   │  → outputs/<method>/ │        │  → clip_only/tip_*/lift  │
   │    best_model.pt     │        │  (KHÔNG cần checkpoint    │
   │            │         │        │   vision → chạy độc lập)  │
   │            ▼         │        └─────────────────────────┘
   │  phase0_reuse        │
   │  (ensemble/tier/     │
   ...
   └─────────────────────┘
            ...
            └───────────────┬────────────────┘
                            ▼
              Gộp outputs/ → đọc comparison.csv + comparison_vlm.csv
```
...
- **TUẦN TỰ (bắt buộc theo thứ tự):**
  ...
  2. `run_all_methods.ipynb` → **rồi mới** `phase0_reuse.ipynb` (phase0 cần `best_model.pt`).

- **SONG SONG (không phụ thuộc nhau, chỉ cần dataset):**
  - Nhánh A (`run_all_methods` → `phase0_reuse`) và nhánh B (`phase2_clip_adapt`) **độc lập** —
    chạy đồng thời ở hai session/máy khác nhau để tiết kiệm thời gian. `phase2` nhanh (vài phút),
    `run_all_methods` lâu (train 6 method × 200 epoch).
...
phase2_clip_adapt.ipynb   (Tip-Adapter + LIFT, track VLM điểm cao nhất)
        ▼
Đọc comparison.csv + comparison_vlm.csv → tinh chỉnh (FUSION_WEIGHTS, MIX_PROB, τ, CLIP_MODEL)
```
